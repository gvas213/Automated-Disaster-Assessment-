"""v8 Gemini: Three-stage CoT with red outline on diff + higher threshold.
...
def process_quartet(pre_img, post_img, post_json) -> list[dict]:
    ...
    output_path = os.path.join(DISASTER_OUTPUT_DIR, f"{base}_v8_gemini_results.json")
    with open(output_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"Results saved to {output_path}")

    build_geojson(post_json, results, f"{base}_v8_gemini_results")
...
def main():
    ...
    print("v8 Gemini: 3-stage CoT + red outline on diff + higher threshold")
    ...
    if all_results:
        ...
        log_accuracy("v8_gemini", quartet_results, all_results)
