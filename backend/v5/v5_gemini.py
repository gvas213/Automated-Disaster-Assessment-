"""v5 Gemini: Diverse-input voting damage assessment.
...
# Vote configurations: (padding, upscale_factor, diff_method, prompt_framing)
VOTE_CONFIGS = [
...
def process_quartet(pre_img, post_img, post_json) -> list[dict]:
    ...
    output_path = os.path.join(DISASTER_OUTPUT_DIR, f"{base}_v5_gemini_results.json")
    with open(output_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"Results saved to {output_path}")

    build_geojson(post_json, results, f"{base}_v5_gemini_results")
...
def main():
    ...
    print("v5 Gemini: diverse-input voting (3 diff methods, 3 paddings, 3 upscales, 3 prompt framings)")
    print(f"Model: {MODEL}")
    ...
    if all_results:
        ...
        log_accuracy("v5_gemini", quartet_results, all_results)
