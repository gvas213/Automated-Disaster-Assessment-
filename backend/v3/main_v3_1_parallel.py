"""main_v3.1: Same pipeline as v3 but parallelizes buildings within each quartet.
...
def process_building(pre_crop, post_crop, uid, ground_truth, gt_features):
    ...
    print(f"    v3.1 says: {prediction['subtype']} (gate={'PASSED' if prediction['gate_passed'] else 'BLOCKED'}, conf={prediction['confidence']:.1f})")
...
def process_quartet(pre_img, post_img, post_json) -> list[dict]:
    ...
    output_path = os.path.join(DISASTER_OUTPUT_DIR, f"{base}_v3_1_results.json")
    with open(output_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"Results saved to {output_path}")

    build_geojson(post_json, results, f"{base}_v3_1_results")
...
def main():
    ...
    print("v3.1 pipeline: v3 + parallel buildings within quartets")
    ...
    if all_results:
        ...
        log_accuracy("main_v3_1", quartet_results, all_results)
