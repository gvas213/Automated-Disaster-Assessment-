"""v3.1 Gemini: Same pipeline as main_v3.1 but using Gemini models.

Parallelism:
  Level 0 — Quartets in parallel (outer pool)
  Level 1 — Buildings within a quartet in parallel (inner pool)
  Gate/severity votes are still sequential per building.
"""
...
def process_building(pre_crop, post_crop, uid, ground_truth, gt_features):
    ...
    print(f"    v3.1-gemini says: {prediction['subtype']} (gate={'PASSED' if prediction['gate_passed'] else 'BLOCKED'}, conf={prediction['confidence']:.1f})")
...
def process_quartet(pre_img, post_img, post_json) -> list[dict]:
    ...
    output_path = os.path.join(DISASTER_OUTPUT_DIR, f"{base}_v3_1_gemini_results.json")
    with open(output_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"Results saved to {output_path}")

    build_geojson(post_json, results, f"{base}_v3_1_gemini_results")
...
def main():
    ...
    print("v3.1 Gemini pipeline: v3 + parallel buildings within quartets")
    ...
    if all_results:
        ...
        log_accuracy("v3_1_gemini", quartet_results, all_results)
