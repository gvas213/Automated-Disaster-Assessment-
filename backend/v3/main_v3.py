"""main_v3: Multi-stage damage assessment with difference images and ensemble voting.

Pipeline:
  Stage 1 — Binary gate (3x voting): Is there structural damage at all?
  Stage 2 — Severity classification (3x voting, full model): minor/major/destroyed
  Stage 3 — Confidence calibration: downgrade low-confidence predictions
"""
...
def call_vlm(model: str, prompt: str, image_paths: list[str]) -> dict:
    """Send a prompt + images to the VLM and parse JSON response."""
...
def run_binary_gate(pre_path: str, post_path: str, diff_path: str) -> bool:
    """Stage 1: Ask the VLM multiple times if the building is damaged. Majority vote."""
    votes = []
    for _ in range(GATE_VOTES):
        try:
            result = call_vlm(GATE_MODEL, BINARY_GATE_PROMPT, [pre_path, post_path, diff_path])
            votes.append(bool(result.get("damaged", False)))
        except Exception as e:
            print(f"      Gate call failed: {e}")
            votes.append(False)  # Conservative: treat failure as no-damage
...
def run_severity_classification(pre_path: str, post_path: str, diff_path: str) -> tuple[str, float]:
    """Stage 2+3: Classify severity with voting and confidence calibration."""
    votes = []
    confidences = []

    for _ in range(SEVERITY_VOTES):
        try:
            result = call_vlm(SEVERITY_MODEL, SEVERITY_PROMPT, [pre_path, post_path, diff_path])
...
def assess_building(pre_crop: str, post_crop: str, uid: str) -> dict:
    """Full pipeline for one building: difference image -> gate -> severity."""
    ...
    # Stage 2+3: Severity classification with calibration
...
def process_quartet(pre_img, post_img, post_json) -> list[dict]:
    """Process a single quartet: crop buildings, run v3 pipeline, return results."""
    ...
    for pre_crop, post_crop, uid, ground_truth in crops:
        ...
        print(f"    v3 says: {prediction['subtype']} (gate={'PASSED' if prediction['gate_passed'] else 'BLOCKED'}, conf={prediction['confidence']:.1f})")
    ...
    output_path = os.path.join(DISASTER_OUTPUT_DIR, f"{base}_v3_results.json")
    with open(output_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"Results saved to {output_path}")

    # Save GeoJSON
    build_geojson(post_json, results, f"{base}_v3_results")
...
def main():
    ...
    num_to_process = min(10, len(quartets))
    print(f"Found {len(quartets)} quartets, processing first {num_to_process}.")
    print("v3 pipeline: difference image + binary gate (3x vote) + severity (3x vote) + confidence calibration")
    ...
    # Use fewer threads — each building makes up to 6 API calls
    ...
    if all_results:
        ...
        log_accuracy("main_v3", quartet_results, all_results)
