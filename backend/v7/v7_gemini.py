"""v7 Gemini: Three-stage CoT with thresholded diff image.
...
def process_quartet(pre_img, post_img, post_json) -> list[dict]:
    ...
    output_path = os.path.join(DISASTER_OUTPUT_DIR, f"{base}_v7_gemini_results.json")
    with open(output_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"Results saved to {output_path}")

    build_geojson(post_json, results, f"{base}_v7_gemini_results")
...
def main():
    ...
    print("v7 Gemini: 3-stage CoT with thresholded+amplified diff")
    ...
    if all_results:
        ...
        log_accuracy("v7_gemini", quartet_results, all_results)
