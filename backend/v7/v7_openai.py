"""v7 OpenAI: Three-stage CoT with thresholded diff image.

Same structure as v6 but with enhanced diff processing:
- Noise below threshold zeroed out (no more false bright pixels)
- Remaining changes amplified (real damage pops)
- Prompts updated to treat diff as the primary signal
...
from v7.difference import compute_difference
from v7.prompts import DESCRIBE_PRE_PROMPT, DESCRIBE_DIFF_PROMPT, EVALUATE_POST_PROMPT
...
def process_building(pre_crop, post_crop, uid, ground_truth, gt_features):
    ...
    print(f"    v7 says: {prediction['subtype']} (conf={prediction['confidence']:.1f})")
...
def process_quartet(pre_img, post_img, post_json) -> list[dict]:
    ...
    output_path = os.path.join(DISASTER_OUTPUT_DIR, f"{base}_v7_openai_results.json")
    with open(output_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"Results saved to {output_path}")

    build_geojson(post_json, results, f"{base}_v7_openai_results")
...
def main():
    ...
    print("v7 OpenAI: 3-stage CoT with thresholded+amplified diff")
    ...
    if all_results:
        ...
        log_accuracy("v7_openai", quartet_results, all_results)
