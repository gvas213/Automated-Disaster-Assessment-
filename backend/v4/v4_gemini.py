"""v4 Gemini: Two-stage CoT damage assessment with upscaling and diff images.
...
def assess_building(pre_crop: str, post_crop: str, uid: str) -> dict:
    ...
    # 2. Difference image on upscaled crops
    diff_path = compute_difference(pre_up, post_up)

    # 3. Stage A: baseline description
    baseline = get_baseline(pre_up)
    print(f"      Baseline: {baseline}")

    # 4. Stage B: classify with voting
...
def process_quartet(pre_img, post_img, post_json) -> list[dict]:
    ...
    output_path = os.path.join(DISASTER_OUTPUT_DIR, f"{base}_v4_gemini_results.json")
    with open(output_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"Results saved to {output_path}")

    build_geojson(post_json, results, f"{base}_v4_gemini_results")
...
def main():
    ...
    num_to_process = min(4 , len(quartets))
    print(f"Found {len(quartets)} quartets, processing first {num_to_process}.")
    print("v4 Gemini: upscale + diff + baseline description + CoT classification (3x vote)")
    ...
    if all_results:
        ...
        log_accuracy("v4_gemini", quartet_results, all_results)
